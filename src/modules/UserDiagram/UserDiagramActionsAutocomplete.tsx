...
export const UserDiagramActionsAutocomplete = observer(() => {
	...
	const handleChangeUser = useCallback((option?: IAutocompleteOption<IUser>) => {
		if (!option) {
			setUser(null);
			return;
		}
	...
	const handleChangeCompareValue = useCallback((option?: IAutocompleteOption<IUser | IDepartment>) => {
		if (!option) {
