...
export const UserDiagramChartTooltip: FC<IUserDiagramChartTooltipProps> = observer(({ point }) => {
  ...
  const renderData = useMemo((): Array<IChartRenderData> => {
    ...
    return chartData
      .filter((d) => Boolean(d))
      .map((d, index) => {
        const { value, rate } = findActivity(d.activity);
        return {
          ...d,
          value,
          rate,
  ...
  return (
    <Stack spacing={2}>
      ...
      <Stack spacing={1}>
        {renderData.map(({ title, rate, value, fill: color }) => {
          const renderData: Array<[string, string]> = [
            ['Activity', value],
            ['Rate', rate],
          ].map(([k, v]) => [String(k), Number(v).toFixed(2)]);
          return (
            <Stack spacing={0} key={title} sx={{ color }}>
              ...
              <Stack spacing={0} sx={{ color: 'inherit' }}>
                {renderData.map((row, index) => (
                  <Stack
                    key={`${title}-${index}-${row.map(v => v.toString()).join(' ')}`}
                    direction='row'
                    spacing={1}
